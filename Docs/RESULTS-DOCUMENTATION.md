...
This file contains scRNA-seq count data containing gene expression values in which rows are genes presented with gene names identifiers (Ensembl IDs, gene names or hgnc_symbol annotation) and columns are cells. We recommend using normalized count data. 

Formats accepted are .h5ad (recommended), .h5, a folder containing a 10x output with mtx/barcode/features files, .csv, .txt, .tsv, .tab and .pickle. For large datasets, do not recommned to input counts in plain formats. Please, input counts as h5ad (recommended), h5 or 10x format with mtx/barcode/features files. 
...
The annotation file is generated by the users after they have annotated each cluster identified by scRNA-seq data. The file must contain two columns: ‘Cell’, indicating the name of the cell or barcode; and ‘cell_type’, indicating the name of the cluster considered. Formats accepted are .csv, .txt, .tsv, .tab and .pickle.
