...
2. **New analysis method added**,  using differentially expressed genes (DEGs) instead of random shuffling (`cellphonedb method degs_analysis`). The user can compute DEGs using their preferred tool, and provide the information to CellphoneDB via text file where the first column in the cell type/cluster and the second column the associated gene id (see [example](https://github.com/ventolab/CellphoneDB/blob/master/in/endometrium_atlas_example/endometrium_example_DEGs.tsv) ), the remaining columns are ignored. We provide [notebooks](https://github.com/ventolab/CellphoneDB/tree/master/notebooks) for both Seurat and Scanpy users. `NOTE THIS METHOD IS UNDER DEVELOPMENT, expected stable release in december 2021.`
