...
2. **New analysis method added**,  using differentially expressed genes (DEGs) instead of random shuffling (`cellphonedb method degs_analysis`). This approach will select interactions where all the members (aka genes) are expressed in a % of cells above `--threshold` and, at least, one gene is a DEG. The user can compute DEGs using their preferred tool, and provide the information to CellphoneDB via text file. The first column should be the cell type/cluster and the second column the associated gene id. The remaining columns are ignored (see [example](https://github.com/ventolab/CellphoneDB/blob/master/in/endometrium_atlas_example/endometrium_example_DEGs.tsv) ). We provide [notebooks](https://github.com/ventolab/CellphoneDB/tree/master/notebooks) for both Seurat and Scanpy users. `NOTE THIS METHOD IS UNDER DEVELOPMENT, expected stable release in december 2021.`
